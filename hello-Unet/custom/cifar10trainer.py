...
class Cifar10Trainer(Executor):
    ...
    def __init__(self,  in_channels, out_channels, lr=1e-4, epochs=5, train_task_name=AppConstants.TASK_TRAIN,
        ...
        self.train_dataloader, _ = split_Train_Val_Data(imgpath, maskpath, batch_size=4)
    ...
    def local_train(self, fl_ctx, weights, abort_signal):
        ...
        for epoch in range(self._epochs):
            _iter = 0
            train_loss_C = 0.0
            print('epoch: ', epoch + 1, ' / ', str(self._epochs))
            print('**************', len(self.train_dataloader), '***************')
            for i, (x, label) in enumerate(self.train_dataloader):
                ...
                loss = self.loss(outputs, label.float())
                loss.backward()
                self.optimizer.step()
            ...
            print('Training epoch: %d / loss_C: %.3f' % (epoch + 1, train_loss_C / _iter))
