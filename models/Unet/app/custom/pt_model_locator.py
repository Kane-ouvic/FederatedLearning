...
class PTModelLocator(ModelLocator):
    ...
    def locate_model(self, model_name, fl_ctx: FLContext) -> Union[DXO, None]:
        if model_name == PTConstants.PTServerName:
            try:
                server_run_dir = fl_ctx.get_engine().get_workspace(
                ).get_app_dir(fl_ctx.get_run_number())
                model_path = os.path.join(
                    server_run_dir, PTConstants.PTFileModelName)
            ...
            self.log_error(
                fl_ctx, f"PTModelLocator doesn't recognize name: {model_name}", fire_event=False)
            return None
